...
RSpec.describe Grumlin::StepData do
  let(:step_data) { described_class.new("step", args: [:arg1, :arg2], params: { key: :value }) }

  describe "#==" do
    subject { step_data == other_step_data }

    context "when name, args and params are equal" do
      let(:other_step_data) { described_class.new("step", args: [:arg1, :arg2], params: { key: :value }) }

      include_examples "returns true"
    end

    context "when name and args are equal, but params are not" do
      let(:other_step_data) { described_class.new("step", args: [:arg1, :arg2], params: { another_key: :another_value }) }
    ...
    context "when araguments are equal, but names are not" do
      let(:other_step_data) { described_class.new("another_", args: [:arg1, :arg2]) }
